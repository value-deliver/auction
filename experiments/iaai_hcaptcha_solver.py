...
if not TWOCAPTCHA_API_KEY:
    print("Warning: TWOCAPTCHA_API_KEY not found in environment variables")
    print("Set it in your .env file or get one from https://2captcha.com/")

TWENTYFOURCAPTCHA_API_KEY = os.environ.get('TWENTYFOURCAPTCHA_API_KEY')
if not TWENTYFOURCAPTCHA_API_KEY:
    print("Warning: TWENTYFOURCAPTCHA_API_KEY not found in environment variables")
    print("Set it in your .env file or get one from https://24captcha.online/")
...
async def solve_hcaptcha_with_challenger(page, site_key, page_url):
    ...
    try:
        ...
        # Initialize AgentT solver
        solver = AgentT.from_page(page=page)

        # Click the hCaptcha checkbox to trigger the challenge
        await solver.handle_checkbox()

        # Manual click at coordinates to ensure checkbox is clicked
        await page.mouse.click(300, 500)
        await asyncio.sleep(2.3)  # Wait for challenge to load

        # Execute the solving process
        print("AgentT solver initialized - executing hCaptcha solving...")
        status = await solver.execute()

        if status and solver.cr and solver.cr.is_pass:
            token = solver.cr.generated_pass_UUID
            if token and len(token) > 10:
                print(f"hcaptcha-challenger solved! Token: {token[:50]}...")
                return token
            else:
                print("No valid token returned by solver - hCaptcha solving failed")
                return False
        else:
            print("Solver execution failed or challenge not passed - hCaptcha solving failed")
            return False
...
async def solve_hcaptcha(sitekey, page_url, page, no_challenger=False, no_free_bypass=False, no_2captcha=False, no_24captcha=False):
    ...
    if not no_challenger and HCAPTCHA_CHALLENGER_AVAILABLE:
    ...
    if not no_free_bypass and PROXY:
    ...
    # Fallback to 2Captcha API
    if not no_2captcha and TWOCAPTCHA_API_KEY:
        try:
            print(f"Solving hCaptcha with sitekey: {sitekey} using 2Captcha API")
            ...
            print("Sending request to 2Captcha API...")
            ...
            if result.get("status") != 1:
                ...
                raise Exception(f"2Captcha API error: {result.get('request')}")
            ...
            for attempt in range(max_attempts):
                ...
                if data.get("status") == 1:
                    ...
                    print(f"Error getting solution: {data.get('request')} {data.get('error_text')}")
                    raise Exception(f"2Captcha solution error: {data.get('request')}")

            print("hCaptcha solving timed out")
            return None

        except Exception as e:
            print(f"Error solving hCaptcha with 2Captcha: {e}")

    # Fallback to 24Captcha API if 2Captcha failed
    if not no_24captcha and TWENTYFOURCAPTCHA_API_KEY:
        print("Trying 24Captcha API as fallback...")
        try:
            solution = solve_hcaptcha_with_24captcha(sitekey, page_url)
            if solution:
                print("hCaptcha solved successfully with 24Captcha!")
                return solution
            else:
                print("24Captcha returned None")
        except Exception as e:
            print(f"Error with 24Captcha: {e}")
    else:
        print("No 24Captcha API key available, skipping fallback")

    print("All CAPTCHA solving methods failed")
    return None

def solve_hcaptcha_with_24captcha(sitekey, page_url):
    """Solve hCaptcha using 24Captcha API"""
    if not TWENTYFOURCAPTCHA_API_KEY:
        print("No 24Captcha API key available")
        return None

    try:
        print(f"Solving hCaptcha with sitekey: {sitekey} using 24Captcha API")

        # Step 1: Send request to get captcha_id
        in_url = "https://24captcha.online/in.php"
        payload = {
            'key': TWENTYFOURCAPTCHA_API_KEY,
            'sitekey': sitekey,
            'pageurl': page_url,
            'json': 1
        }

        print("Sending request to 24Captcha API...")
        response = requests.post(in_url, data=payload, timeout=30)
        result = response.json()
        ...
        # Step 2: Poll for solution
        res_url = "https://24captcha.online/res.php"
        max_attempts = 60  # 5 minutes max
        ...
        print(f"Error solving hCaptcha with 24Captcha: {e}")
        return None
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="IAAI hCaptcha Solver")
    parser.add_argument('--no-challenger', action='store_true', help='Disable hCaptcha Challenger solver')
    parser.add_argument('--no-free-bypass', action='store_true', help='Disable free bypass solver')
    parser.add_argument('--no-2captcha', action='store_true', help='Disable 2Captcha API solver')
    parser.add_argument('--no-24captcha', action='store_true', help='Disable 24Captcha API solver')
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if captcha_detected:
                ...
                solution = await solve_hcaptcha(sitekey, page_url, page, no_challenger=args.no_challenger, no_free_bypass=args.no_free_bypass, no_2captcha=args.no_2captcha, no_24captcha=args.no_24captcha)
